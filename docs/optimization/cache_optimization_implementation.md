    ...
    def get_cached_response(self, query: str, context: str = None, query_type: str = None) -> Optional[Any]:
        ...
        if self.base_cache.redis_cache:
            try:
                ...
                if cached_response:
                    logger.debug(f"智能缓存命中: {query[:30]}...")
                    return cached_response
            except Exception as e:
                logger.error(f"Redis缓存获取失败: {e}")
        
        # 检查内存缓存
        return self.base_cache.get_cache(cache_key)
    ...
    def cache_response(self, query: str, response: Any, context: str = None, query_type: str = None):
        ...
        if self.base_cache.redis_cache:
            try:
                ...
                logger.debug(f"智能缓存设置: {query[:30]}..., TTL: {ttl_seconds}s")
            except Exception as e:
...
class CacheMaintenanceScheduler:
    ...
    def _refresh_hot_cache(self):
        ...
        # 重新缓存热门查询（这里需要具体的查询逻辑）
        logger.info(f"刷新了 {len(hot_queries)} 个热门查询的缓存")
...
def chat():
    ...
    # 处理新查询
    response = chat_handler.handle_chat_message(user_input)
...
这个智能缓存系统将显著提升Chat-AI的性能，特别是在处理重复查询时。通过动态TTL调整和智能预热，可以实现更高的缓存命中率和更快的响应速度。
